...
class PipelineManager:
    ...
    def get_message_tag(self, element_name, sender_name, reciever_name, internal_tag=0):
        tag = self._elements[element_name] + len(self._elements) * self._IDs[sender_name].number + len(
            self._elements) * len(self._IDs) * self._IDs[reciever_name].number + len(self._elements) * len(
            self._IDs) * len(self._IDs) * internal_tag
    ...
    def is_ready_to_send(self, element_name, sender_name, reciever_name, turn, internal_tag=0):
        ...
        key = self.get_message_key(element_name=element_name, sender_name=sender_name, reciever_name=reciever_name,
                                   tag=tag)
        ...
        if turn <= self._last_request_turn[key]:
            if self.verbose:
                _print(
                    f'Pipeline manager {element_name}: {sender_name} at rank {self.get_particles_rank(sender_name)} '
                    f'has already sent a message to {reciever_name} at rank {self.get_particles_rank(reciever_name)} '
                    f'at turn {turn} with tag {tag}')
            return False
        if not self._pending_requests[key].Test():
            if self.verbose:
                _print(
                    f'Pipeline manager {element_name}: {sender_name} at rank {self.get_particles_rank(sender_name)} '
                    f'previous message to {reciever_name} at rank {self.get_particles_rank(reciever_name)} with tag '
                    f'{tag} was not received yet')
            return False
        return True
    ...
    def send_message(self, send_buffer, element_name, sender_name, reciever_name, turn, internal_tag=0):
        ...
        if self.verbose:
            _print(
                f'Pipeline manager {element_name}: {sender_name} at rank {self.get_particles_rank(sender_name)} '
                f'sending message to {reciever_name} at rank {self.get_particles_rank(reciever_name)} at turn '
                f'{turn} with tag {tag}')
        self._pending_requests[key] = self._communicator.Issend(send_buffer,
                                                                dest=self.get_particles_rank(reciever_name), tag=tag)
    ...
    def is_ready_to_recieve(self, element_name, sender_name, reciever_name, internal_tag=0):
        ...
        if self.verbose and not is_ready:
            _print(
                f'Pipeline manager {element_name}: {reciever_name} at rank {self.get_particles_rank(reciever_name)} is '
                f'not ready to recieve from {sender_name} at rank {self.get_particles_rank(sender_name)} with tag {tag}')
        return is_ready

    def receive_message(self, recieve_buffer, element_name, sender_name, reciever_name, internal_tag=0):
        tag = self.get_message_tag(element_name=element_name, sender_name=sender_name, reciever_name=reciever_name,
                                   internal_tag=internal_tag)
        if self.verbose:
            _print(
                f'Pipeline manager {element_name}: {reciever_name} at rank {self.get_particles_rank(reciever_name)} '
                f'recieving from {sender_name} at rank {self.get_particles_rank(sender_name)} with tag {tag}')
