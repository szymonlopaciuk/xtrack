...
inline float_type segment_get_squared_length(LineSegment3D segment) {
...
inline float_type segment_get_length(LineSegment3D segment) {
    return sqrt(segment_get_squared_length(segment));
}


inline Point3D line_segment_point_at_distance(const LineSegment3D segment, const float_type distance)
{
    const float_type length = segment_get_length(segment);

    if (length <= APER_PRECISION)
        return segment.start;

    const float_type t = clamp_value(distance / length, 0.f, 1.f);
    const Point3D direction = point3d_sub(segment.end, segment.start);
    return point3d_add_scaled(segment.start, direction, t);
}


inline Point3D arc_segment_point_at_distance(const ArcSegment3D segment, const float_type distance)
{
    const float_type length = distance;
    ...
    return pose_apply_point(segment.start_in_world, local);
}
...
*/
{
    ...
    // Let A := segment start, compute A - T
    const Point3D p_start = arc_segment_point_at_distance(segment, 0.f);
    const float_type ta_x = p_start.x - t_x;
    const float_type ta_y = p_start.y - t_y;
    const float_type ta_z = p_start.z - t_z;

    // Let B := segment end, compute B - T
    const Point3D p_end = arc_segment_point_at_distance(segment, length);
    ...
    */
    ...
    /* Bisect */
    float_type d_lo = 0.f;
    float_type d_hi = length;
    float_type d_mid = 0.5f * length;

    for (int i = 0; i < 34; i++) {
        d_mid = 0.5f * (d_lo + d_hi);
        const Point3D p_mid = arc_segment_point_at_distance(segment, d_mid);
...
inline float_type segment3d_get_length(const Segment3D segment)
{
    switch (segment.type) {
        case SEGMENT3D_LINE:
...
inline Point3D segment3d_point_at_distance(const Segment3D segment, const float_type distance)
{
    switch (segment.type) {
        case SEGMENT3D_LINE:
            return line_segment_point_at_distance(segment.line, distance);
        case SEGMENT3D_ARC:
            return arc_segment_point_at_distance(segment.arc, distance);
    }
...
inline float_type dist_along_segment3d_where_plane_intersects(const Segment3D segment, const Pose plane)
{
    ...
    switch (segment.type) {
        case SEGMENT3D_LINE:
            return length * line_segment_plane_intersect(segment.line, plane);
        case SEGMENT3D_ARC:
            return length * arc_segment_plane_intersect(segment.arc, plane);
    }
...
inline float_type closest_t_on_segment(const Point3D p, Segment3D seg)
/*
    Closest point parameter t on segment [a,b] to point p.

    // TODO: Implement finding the closest point on arc segments.
*/
{
    const Point3D a = segment3d_point_at_distance(seg, 0);
    const Point3D b = segment3d_point_at_distance(seg, segment3d_get_length(seg));
